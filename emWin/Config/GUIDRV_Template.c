...
//	LCD_PutPixel(x,y,PixelIndex);
//#else	
//	TK80->CMDIR =0x2a; 	__nop();__nop();__nop();__nop();__nop();__nop();__nop();   
//	TK80->DINR =x>>8;  	__nop();__nop();__nop();__nop();__nop();__nop();__nop();
//	TK80->DINR =x; 			__nop();__nop();__nop();__nop();__nop();__nop();
//	TK80->DINR =x>>8;		__nop();__nop();__nop();__nop();__nop();__nop();__nop();
//	TK80->DINR =x;			__nop();__nop();__nop();__nop();__nop();__nop();

//	TK80->CMDIR =0x2b;	__nop();__nop();__nop();__nop();__nop();__nop();__nop();   
//	TK80->DINR =y>>8;		__nop();__nop();__nop();__nop();__nop();__nop();__nop();
//	TK80->DINR =y;			__nop();__nop();__nop();__nop();__nop();__nop();
//	TK80->DINR =y>>8;		__nop();__nop();__nop();__nop();__nop();__nop();__nop();
//	TK80->DINR =y;			__nop();__nop();__nop();__nop();__nop();__nop();

//	TK80->CMDIR =0x2c;	__nop();__nop();__nop();__nop();__nop();__nop();__nop();
...
//  LCD_PIXELINDEX ColorIndex;
  ...
  #if emWin_Optimize
	LCD_DrawLineH(x0, x1, y, LCD_COLORINDEX);
...
#if emWin_Optimize
		#if USE_16bit_LCD
			LCD_DrawHColorLine(x, y, xsize, (u16 *)p);
		#else
			LCD_DrawHColorLine(x, y, xsize, (U32 *)p);
		#endif
#else
...
//{
//#if emWin_Optimize	
////	BlockWrite(x,x+xsize-1,y,y);
////	TK80_DMA_Init(p,xsize);while(TK80->SR & 0x10000);
////	for (;xsize > 0; xsize--) 
////	{
////		TK80->DINR =  *p;
////		p++;
...
static void  _DrawBitLine8BPP(GUI_DEVICE * pDevice, int x, int y, U8 const GUI_UNI_PTR * p, int xsize, const LCD_PIXELINDEX * pTrans) {
  ...
  switch (GUI_pContext->DrawMode & (LCD_DRAWMODE_TRANS | LCD_DRAWMODE_XOR)) {
  case 0:
		BlockWrite(x,x+xsize-1,y,y);//tiky add 20190108
	
    if (pTrans) {
      for (; xsize > 0; xsize--, x++, p++) {
        Pixel = *p;
				TK80->DINR = *(pTrans + Pixel);
...
static void _DrawBitmap(GUI_DEVICE * pDevice, int x0, int y0,
  ...
  case 1:
    ...
    break;
	case 18:
//		BlockWrite(x0,x0+xSize-1,y0,y0+ySize-1);
//		TK80_DMA_Init((const U32 *)pData,xSize*ySize);
//    _DrawBitLine32BPP(pDevice, x0, i + y0, (const U32 *)pData, xSize);
	
		//单点填充写入方式 
	BlockWrite(x0,x0+xSize-1,y0,y0+ySize-1);
	BytesPerLine=  BytesPerLine/4 - xSize;
	p=(int*)pData;
	for (i = 0; i < ySize; i++)
	{
		for(x0 = 0; x0 < xSize; x0++) TK80->DINR = *p++;
		p += BytesPerLine;__nop();__nop();__nop();__nop();__nop();__nop();__nop();
	}
    break;
	case 32:
//		BlockWrite(x0,x0+xSize-1,y0,y0+ySize-1);
...
//	for (i = 0; i < ySize; i++) {
//		DMA2_Channel2->CNDTR = xSize;
//		DMA2_Channel2->CMAR = (u32)pData;
//		DMA2_Channel2->CCR |= 1;
//		TK80->CR |= 1;
//		pData += BytesPerLine;
//		while(!(DMA2->ISR & 1<<5));
...
static void * _GetDevData(GUI_DEVICE * pDevice, int Index) {
  GUI_USE_PARA(pDevice);GUI_USE_PARA(Index);
