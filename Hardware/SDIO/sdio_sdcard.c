...
//升级说明
//V1.1 20160528
//修改SD_WriteDisk和SD_ReadDisk一次读写扇区数大于255个出现错误的bug.
////////////////////////////////////////////////////////////////////////////////// 	 
...
//	if(errorstatus==SD_OK)
//		errorstatus=SD_EnableWideBusOperation(1);	//参数为1，4位宽度,参数为0则为1位宽度；如果是MMC卡,则不能用4位模式 
	
	SDIO_Clock_Set(0);	//设置时钟频率,SDIO时钟计算公式:SDIO_CK时钟=SDIOCLK/[clkdiv+2];其中,SDIOCLK固定为48Mhz
...
//			clkdiv=SDIO_TRANSFER_CLK_DIV;	//SDHC等其他卡，设置最高48/2=24Mhz
			TEST_SDIOx->MMC_IO_MBCTL |= 0x8;//时钟下降沿push，上升沿pull
...
//  TEST_SDIOx->MMC_CTRL |= 0x1<<6;//20191107改动，屏蔽 
} 
//SDIO发送命令函数
//cmdindex:命令索引,低六位有效
//waitrsp:期待的响应.00/10,无响应;01,短响应;11,长响应
//arg:参数
...
//	i = 1000;while(i--);
	...
	{																  
		//SD卡,发送ACMD41 SD_APP_OP_COND,参数为:0x8010000SD_MAX_VOLT_TRIAL0 
		while((!validvoltage)&&(count<3000))   /*****保留问题 count<SD_MAX_VOLT_TRIAL*****/
		{							
			SDIO_Send_Cmd(SD_CMD_APP_CMD,1,0);				//发送CMD55,短响应	 
			errorstatus=CmdResp1Error(SD_CMD_APP_CMD); 	 	//等待R1响应   
...
//			TEST_SDIOx->BUF_CTL |= 1<<15;
			...
			while(!(TEST_SDIOx->CLR_MMC_INT&((1<<2)|(1<<6)|(1<<1))))//CRC/超时/完成(标志)
			{
				...
				{
					*(u32*)tempbuff=TEST_SDIOx->DATA_BUF0;	//循环读取数据
					tempbuff++;
					ob_cnt++;
					timeout=0X7FFFFF;
				} 
...
//	
//	
//	DMA2_Stream3->PAR=(u32)&TEST_SDIOx->FIFO;	//DMA2 外设地址
...
//不再支持SD1.1的卡，一个市面上基本绝种，二个这么低速的卡也没必要再用这种方式来驱动
	{
		temp = 1<<9;
		lsector <<= 9;
	}
